...
| Python drop-in namespace | Rust-backed `quspin.basis` and `quspin.operators` modules | same C ABI used by native Python and Julia | 17/73 files passing unchanged |

## Remaining foundation gaps

| Priority | Required behavior | Rust/core gap | Boundary or Python gap |
|---|---|---|---|
| P0 | Complete general-basis object protocol | finite generated-group closure, reusable deferred reducers, generator words, one-dimensional character phases, and fermionic permutation signs are implemented; higher-dimensional non-Abelian irreps and fermionic particle-hole phases remain | `make_basis=False`, `make`, `representative`, `normalization`, `get_amp`, and generator/sign returns work for the four packed general bases; sector unions remain |
